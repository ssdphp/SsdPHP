...
class Tpl{
	...
	public function __construct(array $config=array())
	{
		...
		require(dirname(__FILE__)."/Tpl.modifier.php");
		require(dirname(__FILE__)."/Tpl.function.php");
	}
	...
	private static function _match($matches){
		...
		self::$_tmpData=array();
		self::$_tmpIndex=0;
		$content = preg_replace_callback("/([\'\"])(.+?)\\1/m",array('SsdPHP\Pulgins\View\Adaptor\Tpl',"_tmpData"),$content);
		...
		$pattern = "/^(\w+)\\s+(.*)/ms";
		do{
			$content = preg_replace_callback($pattern,array('SsdPHP\Pulgins\View\Adaptor\Tpl',"_matchfunction"),$content,-1,$ct);
		}while(0);
		...
		$pattern = "/(.+?)\\|([^\\|]+)/ms";
		do{
			$content = preg_replace_callback($pattern,array('SsdPHP\Pulgins\View\Adaptor\Tpl',"_matchmodifier"),$content,-1,$ct);
		}while($ct);
		//{{{替换变量,加ECHO
		$patterns = array('/\$(\w+)/ms','/^(?!(if|else|for|foreach|elseif))(\w+)([^\=]+)$/ms');
		$replacements=array('SsdPHP\Pulgins\View\Adaptor\Tpl::$_tpl_vars["\1"]','echo \\0');
		$content = preg_replace($patterns,$replacements,$content);
		//还原特殊字符
	...
	private static function _compile($content){
		...
		return preg_replace_callback($pattern,array('SsdPHP\Pulgins\View\Adaptor\Tpl','_match'),$content);
	}
