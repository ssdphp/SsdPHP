...
class Error{
	...
	static function error_handler($errno, $errstr, $errfile, $errline) {
		//session session_regenerate_id bug
		if(stripos($errstr,"session_regenerate_id") !== false){
			return ;
		}
		//swoole client error
		//swoole_client::connect(): connect to server[127.0.0.1:9501] failed. Error: Connection refused[111].
		if(stripos($errstr,"swoole") !== false){
    ...
    public static function getErrorHtml($backtrace,$e=null){
		...
		$arrLen=count($backtrace);
		$html="\r\n".'<table border="1" cellpadding="3" style="font-size: 75%;border: 1px solid #000000;border-collapse: collapse;">
		<tr style="background-color: #ccccff; font-weight: bold; color: #000000;">
		<th >#</th>
		<th >File</th>
		<th >Line</th>
		<th >Class::Method(Args)</th>
		<th>'.(empty($e)?"Error":"Exception").'</th>
		...
		if(!empty($backtrace[$i]['args']) &&!empty($backtrace[$i]['args'][0]) &&!empty($backtrace[$i]['args'][1])){
			//error
			$errorCode = !empty($backtrace[$i]['args'][0])?$backtrace[$i]['args'][0]:"";
			$line = empty($backtrace[$i]['line'])?0:$backtrace[$i]['line'];
			$backtrace[$i]['args'][2] = !empty($backtrace[$i]['args'][2])?$backtrace[$i]['args'][2]:"";
			$html.='<tr style="background-color: #cccccc; color: #000000;">
					<td>'.($index++).'</td>
					<td>'.$backtrace[$i]['args'][2].'</td>
					<td>'.$line.'</td>
					<td></td>
					<td style="font-weight:bold">'
					.(!empty(Error::$error_type[$errorCode])?Error::$error_type[$errorCode]:$errorCode
					).':'.print_r($backtrace[$i]['args'][1],true).'
					</td>
				</tr>';
		}
		if($e){
