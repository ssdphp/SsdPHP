...
class Factory {
    ...
    {
        if(empty($config)){
            $config = SConfig::getField('upload','local');
    ...
    public function upload($files='') {
        ...
        /* 检测上传根目录 */
        if(!self::$uploader->checkRootPath(self::$config['rootPath'])){
            $this->error = self::$uploader->getError();
            return false;
        }

        /* 检查上传目录 */
        if(!self::$uploader->checkSavePath(self::$config['savePath'])){
            $this->error = self::$uploader->getError();
            return false;
        }
        ...
        foreach ($files as $key => $file) {
            ...
            /* 调用回调函数检测文件是否存在 */
            if(is_callable(self::$config['callback'])){
                $data = call_user_func(self::$config['callback'], $file);
                if( self::$config['callback'] && $data ){
                    if ( file_exists('.'.$data['path'])  ) {
                        $info[$key] = $data;
                        continue;
                    }elseif(self::$config['removeTrash']){
                        call_user_func(self::$config['removeTrash'],$data);//删除垃圾据
                    }
                }
            }
            ...
            if(false === $subpath){
                ...
                $file['savepath'] = self::$config['savePath'] . $subpath;
            }
            ...
            /* 保存文件 并记录保存成功的文件 */
            if (self::$uploader->save($file,self::$config['replace'])) {
    ...
    private function getSaveName($file) {
        ...
        /* 文件保存后缀，支持强制更改文件后缀 */
        $ext = empty(self::$config['saveExt']) ? $file['ext'] : self::$config['saveExt'];
