...
class Tpl{
	...
	public function __construct(array $config=array())
	{
		...
		self::$left_delimiter=self::$config['left_delimiter'];
		self::$right_delimiter=self::$config['right_delimiter'];
		self::$template_dir=self::$config['template_dir'];
		self::$compile_dir=self::$config['templates_c'];
		self::$force_compile=self::$config['force_compile'];
	...
	private static function _match($matches){
		...
		self::$_tmpData=array();
		self::$_tmpIndex=0;
		$content = preg_replace_callback("/([\'\"])(.+?)\\1/m",array('SsdPHP\View\Adaptor\Tpl',"_tmpData"),$content);
		//{{{if,elseif,/if; foreach,/foreach; for,/for
		$pattern = "/^(if|foreach|for)(([\s|\(]+)(.+))/msi";
		$content = preg_replace_callback($pattern, function($m){$t = trim($m[3]);$v = trim($m[2]);if(empty($t)){return "{$m[1]}($v){";}else{return "{$m[1]}$v{";}}, $content);
		...
		$pattern = "/^(\w+)\\s+(.*)/ms";
		do{
			$content = preg_replace_callback($pattern,array('SsdPHP\View\Adaptor\Tpl',"_matchfunction"),$content,-1,$ct);
		}while(0);
		...
		$pattern = "/(.+?)\\|([^\\|]+)/ms";
		do{
			$content = preg_replace_callback($pattern,array('SsdPHP\View\Adaptor\Tpl',"_matchmodifier"),$content,-1,$ct);
		}while($ct);
		//{{{替换变量,加ECHO
		$patterns = array('/\$(\w+)/ms','/^(?!(if|else|for|foreach|elseif))(\w+)([^\=]+)$/ms');
		$replacements=array('SsdPHP\View\Adaptor\Tpl::$_tpl_vars["\1"]','echo \\0');
		$content = preg_replace($patterns,$replacements,$content);
		//还原特殊字符
	...
	private static function _matchfunction($function){
		...
		if($r){
			$func='SsdPHP\View\Adaptor\tpl_function_'.$function[1];
	...
	private static function _matchmodifier($modifier){
		...
		if($r){
			...
			$func_m = 'SsdPHP\View\Adaptor\tpl_modifier_'.$func;
	...
	private static function _compile($content){
        ...
        //替换有内容的block content到data
        $pattern1="/<block name=\"content\">(.+)<\/block>/msU";
        preg_match($pattern1,$content,$block);
        if(empty(self::$block['content']) && !empty($block[1]) ){
            self::$block['content'] = $block[1];
            $content = preg_replace($pattern1,"",$content);

        }
        if(empty(self::$block['replace_block'])){
            $pattern="/<block name=\"data\"><\/block>/";
            preg_match($pattern,$content,$flag);
            if(!empty($flag) && !empty(self::$block['content'])){
                self::$block['replace_block'] = $pattern;
                $content = preg_replace($pattern,self::$block['content']."\r\n",$content);
            }
        }
		$pattern="/{$left_delimiter_quote}([\S].*){$right_delimiter_quota}/msU";
		return preg_replace_callback($pattern,array('SsdPHP\View\Adaptor\Tpl','_match'),$content);
	}
